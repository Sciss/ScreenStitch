...
{ // AbstractMultiSlider
	...
	def setHorizontalEditMode( mode: Int ) {
		horizEditMode = mode
	}
	
	def setLockBounds( onOff: Boolean ) {
		lockBounds = onOff
	}
	...
	def setDrawLines( onOff: Boolean ) {
		drawLines	= onOff
		repaint()
	}
	...
	protected def fireActionPerformed() {
		val l = al
		if( l != null ) {
	...
	def setClipThumbs( onOff: Boolean ) {
		if( onOff != clipThumbs ) {
			clipThumbs = onOff
	...
	def setSelectionColor( c: Color ) {
		selectionColor = c
	}
	
	def setLabel( index: Int, label: String ) {
		if( index == -1 ) {
	...
	def setShape( index: Int, shape: Int, curve: Float ) {
		if( index == -1 ) {
			for( i <- (0 until nodes.length) ) {
				nodes( i ).shape = shape
				nodes( i ).curve = curve
...
//	}
	...
	def setStepSize( stepSize: Float ) {
	...
	def setReadOnly( index: Int, readOnly: Boolean ) {
		if( index == -1 ) {
			for( i <- (0 until nodes.length) ) {
	...
	def setSelected( index: Int, selected: Boolean ) {
		if( index == -1 ) {
			for( i <- (0 until nodes.length) ) {
	...
	def setStrokeColor( c: Color ) {
		strokeColor	= c
		repaint()
	}

	def setStrokeWidth( px: Float ) {
		stroke = if( px != 1 ) new BasicStroke( px ) else null
		repaint()
	}

	def setThumbSize( index: Int, size: Float ) {
		if( index == -1 ) {
			for( i <- (0 until nodes.length) ) {
	...
	def setThumbWidth( index: Int, w: Float ) {
		if( index == -1 ) {
			for( i <- (0 until nodes.length) ) {
	...
	def setThumbHeight( index: Int, h: Float ) {
		if( index == -1 ) {
			for( i <- (0 until nodes.length) ) {
	...
	def setValues( x: Array[ Float ], y: Array[ Float ]) {
	...
	def setValues( x: Array[ Float ], y: Array[ Float ], shapes: Array[ Int ], curves: Array[ Float ]) {
		val newNumVals = x.length
		val oldNumVals = nodes.length
	...
	def setConnections( index: Int, targets: Array[ Int ]) {
	...
	def setConnectionsUsed( onOff: Boolean ) {
